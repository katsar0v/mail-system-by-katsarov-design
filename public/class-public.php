...
class MSKD_Public {
	...
	public function init() {
		...
		add_shortcode( 'mskd_form_gallery', array( $this, 'form_gallery_shortcode' ) );
	...
	/**
	 * Form gallery shortcode - displays available subscription forms with shortcodes
	 *
	 * This is a read-only page that lists all available mailing lists with their
	 * corresponding shortcodes for easy copying and embedding.
	 *
	 * @param array $atts Shortcode attributes.
	 * @return string
	 */
	public function form_gallery_shortcode( $atts ) {
		// Enqueue assets only when shortcode is used.
		$this->enqueue_assets();

		$atts = shortcode_atts(
			array(
				'title' => __( 'Available Subscription Forms', 'mail-system-by-katsarov-design' ),
			),
			$atts
		);
